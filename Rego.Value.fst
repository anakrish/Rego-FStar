...
open FStar.String
  ...
  let rec array_cmp (l1:list value) (l2:list value) : Tot ord (decreases l1)=
    match l1, l2 with
      | hd1::tl1, hd2::tl2  ->
  ...
  and object_cmp (l1: list (value * value)) (l2 : list (value*value)) : Tot ord (decreases l1) =
    match l1, l2 with
      | (k1, v1)::tl1, (k2, v2)::tl2 ->
  ...
  let rec insert_into_set_items (items:list value) (item:value) =
   ...
   let insert_into_set (set:value) (item:value) : value =
       match set with
   ...
   let rec lookup_path (obj:value) (path:list value)
       ...
       match obj, path with
         ...
         | Object _, p::tl ->
           (match lookup_path obj (p::[]) with
             ...
             | Undefined ->
               (* create subobject *)
               let subobj = insert_at_path (Object []) tl v in
                 (* insert subobj as field p into obj *)
                 insert_into_object obj p subobj

             (* The key p exists in obj *)
             | subobj ->
               (* insert v at path tl into subobj *)
               let subobj1 = insert_at_path subobj tl v in
                 (match subobj1 with
                   (* insertion failed *)
                   | Undefined -> Undefined
                   (* insertion succeeded. Insert subobj as field p in obj *)
                   | _ -> insert_into_object obj p subobj))
  ...
  and field_to_json (a:list (value*value)) : Tot (list string) (decreases a) =
    ...
    | [] -> []
    | (k, v)::tl ->
      let k_json = match k with
        | String s -> Printf.sprintf "\"%s\"" (escaped s)
        | _ -> Printf.sprintf "\"%s\"" (escaped (to_json k)) in
  ...
  and field_to_json_p (a:list (value*value)) (indent:string) : Tot (list string) (decreases a) =
    ...
    | (k, v)::tl ->
      let k_json = match k with
        ...
        | _ -> Printf.sprintf "%s\"%s\"" indent (escaped (to_json k)) in
